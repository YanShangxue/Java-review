...
public class BookManageInterFrm extends JInternalFrame {
	...
	private void bookUpdateActionPerformed(ActionEvent evt) {
		...
		if(StringUtil.isEmpty(author)){
			JOptionPane.showMessageDialog(null, "书籍作者不能为空！");
			return;
		}
		
		if(StringUtil.isEmpty(price)){
			JOptionPane.showMessageDialog(null, "书籍价格不能为空！");
			return;
		}
		...
		try{
			...
			if(addNum==1){
				JOptionPane.showMessageDialog(null, "书籍修改成功！");
				resetValue();
				this.fillTable(new Book());
			}else{
				JOptionPane.showMessageDialog(null, "书籍修改失败！");
			}
		}catch(Exception e){
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "书籍修改失败！");
